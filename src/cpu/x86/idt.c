 ...
 * Updated: timer IRQ calls scheduler_tick() for preemptive scheduling.
 */
...
void exception_handler(uint32_t exception_num, uint64_t error_code) {
    ...
    /* If a user process caused this, kill it and reschedule */
    ...
    if (cur && cur != scheduler_get_idle()) {
        ...
        process_mark_zombie(cur, -exception_num);
        __asm__ volatile("sti");
        schedule();
        return;
    }
 ...
 *
 * NOTE: scheduler_tick() is called BEFORE pic_send_eoi() so that the
 * context_switch() inside schedule() (if triggered) happens while the
 * PIC's in-service bit is still set.  EOI is issued by the new process
 * when it returns from the interrupt.  This is the correct approach for
 * a simple non-nested IRQ model.
 */
void irq_handler(uint32_t irq_num) {
    ...
    switch (irq_num) {
        case 0:  /* Timer – advance tick counter then let scheduler decide */
            timer_handler();
            scheduler_tick();   /* ← preemptive scheduling hook */
            break;
