...
/*
 * idt_print_info - write the IDTR base and limit to VGA.
 */
void idt_print_info(void) {
    vga_writestring("\nIDT Information:\n");
    vga_writestring("  Base:    0x"); print_hex(idt_pointer.base);         vga_writestring("\n");
    vga_writestring("  Limit:   ");   print_dec(idt_pointer.limit + 1);    vga_writestring(" bytes\n");
    vga_writestring("  Entries: ");   print_dec(IDT_ENTRIES);              vga_writestring("\n");
}

/*
 * idt_print_stats - write per-exception and per-IRQ fire counts to VGA.
 * Only active (non-zero) entries are printed.
 */
void idt_print_stats(void) {
    ...
    for (int i = 0; i < 32; i++) {
        if (exception_counts[i] == 0) continue;
        vga_writestring("  "); vga_writestring(exception_names[i]);
        vga_writestring(": "); print_dec(exception_counts[i]);
        vga_writestring("\n");
    }

    vga_writestring("Hardware IRQs:\n");
    for (int i = 0; i < 16; i++) {
        if (interrupt_counts[32 + i] == 0) continue;
        vga_writestring("  IRQ"); print_dec((uint64_t)i);
        vga_writestring(" (");    vga_writestring(irq_names[i]);
        vga_writestring("): ");   print_dec(interrupt_counts[32 + i]);
        vga_writestring("\n");
    }
}
