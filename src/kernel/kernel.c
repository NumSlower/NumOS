...
extern void cmd_cat(const char *args);
extern void cmd_pwd(void);
extern void cmd_mkdir(const char *args);
extern void cmd_rmdir(const char *args);
extern void cmd_rm(const char *args);
extern void cmd_touch(const char *args);
extern void cmd_du(const char *args);
extern void cmd_mount(const char *args);
extern void cmd_umount(void);
extern void cmd_fsinfo(void);
extern void cmd_drives(void);
extern void cmd_hexdump(const char *args);

// Enhanced process_command function - replace your existing one
void process_command(const char *command) {
    ...
    if (strcmp(cmd, "mount") == 0) {
        ...
        cmd_du(args);
    } else if (strcmp(cmd, "hexdump") == 0 || strcmp(cmd, "hd") == 0) {
        cmd_hexdump(args);
    ...
    // System Commands
        ...
        vga_writestring("System Commands:\n");
        ...
        vga_writestring("\nFile System Commands:\n");
        vga_writestring("  drives       - Show available disk drives\n");
        vga_writestring("  mount [n]    - Mount FAT32 file system from drive n\n");
        vga_writestring("  umount       - Unmount file system\n");
        vga_writestring("  fsinfo       - Show file system information\n");
        vga_writestring("  ls [-l] [-a] [path] - List directory contents\n");
        vga_writestring("  cd <path>    - Change directory\n");
        vga_writestring("  pwd          - Show current directory\n");
        vga_writestring("  cat <file>   - Display file contents\n");
        vga_writestring("  hexdump <file> - Show hex dump of file\n");
        vga_writestring("  mkdir <dir>  - Create directory\n");
        vga_writestring("  touch <file> - Create empty file\n");
        vga_writestring("  rm <file>    - Remove file\n");
        vga_writestring("  du [path]    - Show disk usage\n");
        vga_writestring("\nUtility Commands:\n");
        ...
        vga_writestring("NumOS Version 3.0 - File System Edition\n");
        vga_writestring("64-bit Operating System with FAT32 Support\n");
        vga_writestring("Features:\n");
        ...
        vga_writestring("- ATA/IDE disk driver\n");
        vga_writestring("- FAT32 file system support\n");
        vga_writestring("- Unix-like shell commands\n");
        vga_writestring("- Built with C and Assembly\n");
    } else if (strncmp(cmd, "echo ", 5) == 0) {
        if (args) {
            vga_writestring(args);
        }
        vga_putchar('\n');
    ...
    } else if (strcmp(cmd, "pagingstats") == 0) {
        paging_print_stats();
    } else if (strcmp(cmd, "vmregions") == 0) {
    ...
    } else if (strncmp(cmd, "translate ", 10) == 0) {
        if (!args) {
            vga_writestring("translate: missing address argument\n");
        } else {
            // Simple hex parser for virtual address
            const char *hex_str = args;
        ...
        if (!args) {
            vga_writestring("sleep: missing duration argument\n");
        } else {
            // Parse sleep duration
            uint32_t ms = 0;
            const char *ms_str = args;
