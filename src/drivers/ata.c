 ...
 * ATA bus.  Sector-level DMA is not implemented; all transfers use the
 ...
 *   ata_write_sectors()   - write up to 255 sectors to an LBA address
 *   ata_flush_cache()     - issue CACHE FLUSH so writes reach the media
 *
 * Low-level helpers:
...
int ata_identify(struct ata_device *dev) {
     ...
     * guard in ata_read_sectors / ata_write_sectors.
     *
...
/*
 * ata_write_sectors - write count sectors to LBA address lba from buffer
 * using 28-bit LBA PIO mode.
 *
 * Follows the write with a CACHE FLUSH command to ensure data reaches the
 * media.
 * Returns 0 on success, -1 on error.
 */
int ata_write_sectors(struct ata_device *dev,
    ...
    /* Flush write cache to ensure data is committed to the media */
...
/*
 * ata_flush_cache - send the CACHE FLUSH command to commit pending writes.
 * Returns 0 on success, -1 if the device is absent or the command fails.
 */
